...
1. `python3 day2_aicc/app.py --scenario order_status` 실행 후 `code path` 따라가기
2. LangGraph checkpoint 중단/재개 확인
3. direct / indirect prompt injection 비교
4. action_guard로 쓰기 tool boundary 확인
5. `python3 day2_aicc/eval_day2.py --compare-models`로 cheap/standard/strong 품질·비용 비교
...
Day 2는 기본값으로 실제 Gemini LLM을 호출하므로 `GEMINI_API_KEY`가 필요해요. 네트워크/API 문제를 분리해서 graph만 볼 때는 `--llm-mode mock`을 써요.
...
Day 2는 TODO가 많은 편이에요. 전체를 다 끝내는 방식보다, 조별로 TODO를 나눠서 마지막 5분에 결과를 공유하는 방식이 좋아요.
