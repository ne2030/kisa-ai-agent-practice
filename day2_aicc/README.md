...
Day 2 graph의 node가 전부 agent는 아니에요. `specialist`만 LLM agent 역할을 하고, 나머지는 host application이 실행하는 Python node예요.

![Day 2 AICC structure](./diagrams/aicc_structure.svg)

- `graph.py`: node 순서만 보여주는 얇은 wiring 파일
- `nodes/routing.py`: `triage` router. agent가 아니에요.
- `nodes/context.py`: 주문/고객/배송/정책을 읽어오는 곳
- `nodes/specialist.py`: 답변 초안과 `proposed_actions`를 만드는 LLM agent node
- `guardrails.py`: input/context/action guard
- `nodes/actions.py`: guard를 통과한 tool 실행과 최종 답변
...
| `intent/order: order_status / ORD-1001` | `nodes/routing.py::triage_node()` | `parse_intent()`가 “주문” 문구를 보고 `order_status`로 분류하고, 정규식으로 주문번호를 뽑아요. |
| `actions: (none)` | `nodes/specialist.py::mock_specialist_node()` 또는 `live_llm.py::live_specialist_node()` | 주문 조회는 읽기 응답이라 `proposed_actions`가 비어 있어요. |
| `risk events` | 각 node의 `append_risk_event()` / `append_event()` | graph가 지나온 node 기록이에요. `action_guard:no_action`이면 실행할 쓰기 tool이 없었다는 뜻이에요. |
...
- `nodes/*.py::*_node()`: state를 읽고 일부 필드만 업데이트하는 함수
...
| `graph.py` | LangGraph node/edge wiring | `build_graph` |
| `nodes/specialist.py` | mock/live specialist 분기 | `specialist_node`, `mock_specialist_node` |
...
1. `nodes/routing.py`의 `TODO-D2-01` — `교환`, `분실`, `부분 취소` 중 하나를 새 intent로 추가
2. `nodes/context.py`의 `TODO-D2-02` — 다른 고객 주문 조회 실패 scenario 추가
3. `nodes/context.py`의 `TODO-D2-03` — strict budget에서 policy doc 수를 줄였을 때 pass/fail 변화 기록
4. `nodes/specialist.py`의 `TODO-D2-04` — cheap model이 shipped 주문에 잘못 action 제안하는 부분 수정
5. `nodes/specialist.py`의 `TODO-D2-05` — 보상 쿠폰 앞에 human approval node 추가 설계 또는 구현
