...
from .guardrails import action_guard_node, block, input_guard_node, sanitize_policy_docs
from .mock_data import TODAY
from .model_policy import estimate_cost, resolve_policy, route_model_for_intent
from .state import AICCState, IntentName, ProposedAction, ToolCall
from .tools import (
    TOOL_REGISTRY,
    get_customer,
    get_order,
...
def _parse_intent(message: str) -> IntentName:
    ...
    if any(k in message for k in ["교환", "exchange"]):
...
def mock_specialist_node(state: AICCState) -> dict[str, Any]:
    ...
    if intent == "order_status":
        ...
        age = _days_since_delivery(order)
        if order.get("status") == "delivered" and age is not None and age <= 14:
            actions.append(
                {
                    "tool": "create_exchange_request",
                    "args": {"order_id": order["order_id"], "reason": "customer exchange request"},
                    "reason": "exchange request within policy window",
                    "risk": "high",
                }
            )
            draft = "배송 완료 후 14일 이내라 교환 접수를 생성하겠습니다."
        else:
            actions.append(
                {
                    "tool": "create_support_ticket",
                    "args": {"customer_id": customer["customer_id"], "order_id": order["order_id"], "reason": "exchange policy review"},
                    "reason": "manual exchange review",
                    "risk": "low",
                }
            )
            draft = "자동 교환 기준에 바로 해당하지 않아 상담 티켓으로 검토하겠습니다."
