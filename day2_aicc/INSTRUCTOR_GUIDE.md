...
checkpoint는 Day 1과 같은 용도로 `day2_aicc/solutions/`에 있습니다.
...
Day 1에서는 `agent.py` 안에서 ReAct loop를 직접 확인했습니다. Day 2에서는 같은 아이디어를 고객응대 서비스 흐름으로 나눠 봅니다. 이제는 답변 생성만 보지 않고, 어떤 상태를 저장할지, 어떤 action을 막을지, 어떤 모델을 어디에 쓸지까지 같이 확인합니다.
