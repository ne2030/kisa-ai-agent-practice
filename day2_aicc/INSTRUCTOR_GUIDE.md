...
Day 1에서는 `agent.py` 하나 안에서 ReAct loop가 어떻게 도는지 봤어요. Day 2에서는 같은 아이디어를 고객응대 서비스 흐름으로 나눠 봐요. 핵심은 “답변 생성” 하나가 아니라, 상태 관리, tool action, guardrail, model routing, checkpoint, evaluation이 같이 돌아가는 구조예요.

Day 2 실습은 커맨드 실행만으로 끝나지 않아요. 각 실행 결과 맨 아래의 `code path`를 따라가며 파일을 열고, 어떤 node가 어떤 state 필드를 만들었는지 확인해요. 처음 보는 사람은 `day2_aicc/CODE_WALKTHROUGH.md`를 옆에 열어두면 흐름을 놓치기 어려워요.
...
실습 시간은 명령어만 실행하는 시간이 아니에요. 각 구간에서 **코드 위치 확인 → 실행 → 결과 비교 → TODO 수정** 순서로 진행해요.
...
Day 2 기본값은 실제 Gemini LLM 호출이에요. `.env`에 `GEMINI_API_KEY`가 필요해요.

```bash
python3 day2_aicc/app.py --scenario order_status
```

API key나 네트워크 문제를 분리해서 구조만 확인할 때는 mock mode를 써요.
...
## 0–10분 · LangGraph와 코드 구조부터 잡기

먼저 LangGraph가 무엇을 해주는지 확인해요. node 함수는 현재 state를 받고, 바뀐 필드만 dict로 반환해요. `build_graph()`는 그 node들을 어떤 순서로 실행할지 적어둔 곳이에요.
...
- 출력 맨 아래의 `code path`가 실제로 어떤 파일을 가리키는지 확인
...
- `--show-state`를 붙여서 checkpoint에 남는 state 크기와 민감정보를 확인해요.
- state에 원문 개인정보나 긴 RAG context를 그대로 남기는 것이 왜 위험한지 적어 봐요.

---

## 27–39분 · Tool boundary 수정
...
- `TODO-D2-04`: shipped 주문에 대해 cheap profile이 잘못 action을 제안하는 부분을 수정해요.
- `TODO-D2-09`: 배송지 변경 가능 조건을 `processing + 송장번호 없음`으로 강화해요.
- 수정 후 `guards=off`에서도 더 안전해졌는지 다시 비교해요.

---

## 39–50분 · Direct / indirect prompt injection
...
- `TODO-D2-07`: 한국어 direct injection 패턴을 추가해요.
- `TODO-D2-08`: HTML comment, markdown link, base64 힌트 같은 indirect payload 패턴을 추가해요.
- context guard만 켠 경우와 context+action guard를 같이 켠 경우를 비교해요.

---

## 50–57분 · Safety metrics / model cost
...
- `TODO-D2-06`: `auto` routing 정책을 수정해요.
  - 조회성 intent → cheap
  - 배송지 변경 → standard
  - 환불/보상 → strong
- `eval_day2.py`에 새로운 case를 하나 추가하고 ASR/FPR/Utility가 어떻게 바뀌는지 확인해요.

---

## 57–60분 · Mini challenge

하나를 골라서 graph와 eval을 같이 바꿔요.
...
Day 2는 `graph.py`, `guardrails.py`, `model_policy.py`, `eval_day2.py`가 함께 바뀌므로 Day 1처럼 단일 파일 snapshot이 아니라 패키지 폴더 snapshot으로 제공해요.
...
Agent 운영에서는 모델 성능만 보지 않아요. 상태를 어디까지 저장할지, 어떤 context를 믿을지, 어떤 tool을 누가 승인할지, 어떤 요청을 어떤 모델에 보낼지까지 정해야 해요. Day 2 실습 코드는 그 결정을 작게 압축한 샘플이에요.
