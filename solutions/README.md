...
실습은 `agent.py` 하나를 점진적으로 바꿔가며 진행합니다. 이 폴더의 파일은 강의자가 막힌 학생에게 필요한 단계만 보여주기 위한 checkpoint입니다.

## 진행 시나리오

| 단계 | 실행/비교 파일 | 학생이 보는 변화 | Langfuse에서 보이는 것 |
|---|---|---|---|
...
| Step 3 parent observe | `solutions/03_nested_trace.py` | `react_loop`에 `@observe()` 한 줄 추가 | `react_loop` 하나 아래 `llm.generate_content` → `tool.execute` → `llm.generate_content`가 nested로 묶임 |
| Step 4 metadata | `solutions/04_metadata.py` | `get_client().update_current_span(metadata=...)` 추가 | trace tree는 같고, metadata/tags/user_id로 본인 실행을 찾기 쉬워짐 |
