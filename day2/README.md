...
기본 case는 정책 충돌과 예외가 섞인 `policy-conflict-resolution`이에요. 같은 모델에서도 `naive`와 `structured`는 출력 형식, 기준어 누락, token 수가 다르게 나와요.
...
- `naive`는 짧고 편하지만 정책 예외와 확인 항목이 흔들릴 수 있어요.
