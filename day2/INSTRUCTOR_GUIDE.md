...
1. `naive`는 input token이 적고 비용도 낮지만, 필요한 기준어와 구조가 빠질 수 있어요.
2. `structured`는 input token이 늘어 비용이 조금 올라가지만, 답변 형식과 판단 기준을 더 안정적으로 고정해요.
3. 비용 비교는 모델 가격만 보는 게 아니라 prompt 길이, output 길이, latency, eval 점수를 같이 봐야 해요.
...
naive
- 결론은 맞힐 수 있음
- 최신 상태나 상태값 일부가 빠질 수 있음
- eval에서 missing terms가 나올 수 있음
