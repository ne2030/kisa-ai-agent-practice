...
같은 profile에서 prompt style을 바꿔 실행하고 eval 결과를 확인해요.

```bash
python3 day2/cost_lab.py --case policy-conflict-resolution --profile standard --prompt-style naive
python3 day2/cost_eval.py --report day2/reports/cost_latest.json

python3 day2/cost_lab.py --case policy-conflict-resolution --profile standard --prompt-style structured
python3 day2/cost_eval.py --report day2/reports/cost_latest.json
```

볼 것:

- `naive`는 편하게 요청한 요약이에요. 정책 예외나 내부 확인 항목이 빠질 수 있어요.
- `structured`는 역할, 출력 형식, 금지할 행동, 보존할 정보를 같이 줘요.
- eval 결과에서 정확성, 근거성, 활용성 점수와 missing terms를 비교해요.

---

## 15–23분 · Cost model 직접 비교

structured prompt에서 profile을 바꿔 실행하고 eval 결과를 확인해요.

```bash
python3 day2/cost_lab.py --case policy-conflict-resolution --profile cheap --prompt-style structured
python3 day2/cost_eval.py --report day2/reports/cost_latest.json

python3 day2/cost_lab.py --case policy-conflict-resolution --profile strong --prompt-style structured
python3 day2/cost_eval.py --report day2/reports/cost_latest.json
```
...
같은 profile과 prompt style을 2회 실행해요.

```bash
python3 day2/cost_repeat.py --case policy-conflict-resolution --profile standard --prompt-style naive --runs 2
python3 day2/cost_repeat.py --case policy-conflict-resolution --profile standard --prompt-style structured --runs 2
```
